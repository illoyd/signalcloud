FactoryGirl.define do
  ...
  factory :freshbooks_account, parent: :account do
    ...
    after(:create) do |account, evaluator|
      ...
      create_tickets_for_month( stencil, '2012-11-01', 4 )
      
      # January 2013: 3 of each (15; 33 ledger_entries)
      create_tickets_for_month( stencil, '2013-01-01', 3 )
    end
  end
  
end

def create_tickets_for_month( stencil, month, count )
  month = month.to_datetime unless month.is_a? DateTime
  month_start = month.beginning_of_month
  month_end = month.end_of_month - 1.hour
  
  count.times { open_and_configure_ticket( stencil, false, rand_datetime( month_start, month_end ), Ticket::CHALLENGE_SENT ) }
  [ Ticket::CONFIRMED, Ticket::FAILED, Ticket::DENIED, Ticket::EXPIRED ].each do |status|
    count.times { open_and_configure_ticket( stencil, true, rand_datetime( month_start, month_end ), status ) }
  end
end
...
def open_and_configure_ticket( stencil, settled=false, date_sent=nil, status=nil, outbound_price=0.04, inbound_price=0.01 )
  ...
  if [ Ticket::CONFIRMED, Ticket::DENIED, Ticket::FAILED ].include? ticket.status
    ticket.response_received_at = ticket.challenge_sent_at + rand_in_range( 1, stencil.seconds_to_live )
  ...
  if [ Ticket::EXPIRED ].include? ticket.status
    ticket.reply_sent_at = ticket.challenge_sent_at + stencil.seconds_to_live + rand_in_range( 1, 5 )
    message = create( :reply_message, ticket: ticket, provider_response: { test: 'test' }, sent_at: ticket.reply_sent_at )
    if settled
      message.provider_cost = outbound_price
      message.save!
